...
public:
    ...
    Handle *insert(int key, T *value)
    {
        ...
        // Decrease the key of the node, the original value is 
    ...
    T *extractMin(void)
    {
        ...
        // Remove the last element at the end of the vector, 
        // which is v[v.size()-1])
        v.pop_back();
        // Fix the heap
        minHeapfy(0);
    ...
    bool decreaseKey(Handle *h, int newkey)
    {
        ...
        if (h->getIndex() == -1 || 
            v[h->getIndex()]->getKey() <= newkey)
            return false;
        // Set the newkey to the node, replace "key" by "newkey"
        v[h->getIndex()]->setKey(newkey);
        // Fixup the queue, and return true, if the parent is larger, 
        // we do the swap operation
        while (h->getIndex() > 0 && 
        v[parent(h->getIndex())]->getKey() > v[h->getIndex()]->getKey())
            swapPQNode(v[h->getIndex()],v[parent(h->getIndex())]);
        // Otherwise, replace "key" by "newkey", fixup the queue, 
        // and return true.
        return true;
    }
...
private:
    ...
    // min heapify function, used to fix one element violate the 
    // heap property, then propergate down
